...
func main() {
	db, err := sql.Open("mysql", "root:root@tcp(10.10.10.193:3307)/curso_go")
	if err != nil {
		panic(err)
	}
	defer db.Close()
	product := NewProduct("notebook", 7000.0)
	err = insertProduct(db, product)
	if err != nil {
		panic(err)
	}

	product.Price = 5500.99
...
func insertProduct(db *sql.DB, product *Product) error {
	stmt, err := db.Prepare("insert ignore into products (id, name, price) values (?, ?, ?)")
	if err != nil {
		return err
	}
	defer stmt.Close()
